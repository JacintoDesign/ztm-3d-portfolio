...
| **Reflector strip** | An **L**, at **`y = 0.004`**: the alley `x ∈ [-6, 6]` from `z = -26` to a cut parallel to §3.1's bend face, plus an arm `x ∈ [1.35, 6]` running on to `z = 33` for §3.6's carriageway. Was a 12 × 52 rectangle, then 12 × 59 — see below for why it stopped being one |
| **Cut clearance** | `BEND_KERB.depth − 0.05` — §3.1's pavement width, less a tuck so the mirror runs **under** the kerb instead of butting against it |
...
**And what fills the gap is §3.1's pavement rather than floor, which is the other half of the fix and it took three goes.**
...
Colour and roughness were both tried on attempt 1 and both made it worse: **the band's brightness was never the fault; the band was.** And attempt 2 says the same thing one level up — *the band's depth* was the fault, because at 0.40 m it was still a band. `bendClearance` reads `BEND_KERB.depth`, so the mirror stops where the pavement starts and the two cannot drift apart.

**Less 5 cm, and that is the third time this section has learned the same lesson.** Cut flush, the reflector's edge and the kerb's inner face are one line in plan, and one line is where floating-point disagreement leaves a hairline of base plane at grazing incidence — present on some frames and not others, which is worse than present on all of them. Tucked, the mirror runs *under* the kerb and the overlap is hidden by a 0.118 m step from any eye above it. **Reflective floor that cannot be seen cannot leak**, which is the same sentence as *whether reflective floor is there at all is the only lever*, read from the other side.
